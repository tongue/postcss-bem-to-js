  ...
  return root => {
    ...
    const setOutput = (inputPath, value) => {
      ...
      const transformedPath = path.map((p, i) =>
        sanitise(
          i === 0 ? `${prefix}${blockKeyTransform(p)}` : elementKeyTransform(p)
        )
      );
        ...
        if (eloc > 0) {
          if (mloc > eloc) {
            ...
            const [block, element, modifier] = [
              ...
              className.substr(eloc + edel.length, mloc - eloc - edel.length),
              className.substr(mloc + mdel.length)
            ];
            setOutput([block, element, `$${modifier}`], `${className}`);
          } else {
            // Element with no modifier
            const [block, element] = className.split(edel);
            setOutput([block, element, "$"], className);
          }
        } else if (mloc > 0) {
          // Block with modifier
          const [block, modifier] = className.split(mdel);
          setOutput([block, `$${modifier}`], className);
        } else {
          // Block with no modifier
          setOutput([className, "$"], className);
        }
      }
    };

    const processor = selectors => selectors.walkClasses(processClassNode);
